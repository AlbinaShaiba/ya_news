...
def test_user_cant_use_bad_words(author_client, delete_url_reverse):
    bad_words_data = {'text': f'Какой-то текст, {choice(BAD_WORDS)}, еще текст'}
    Comment.objects.all().delete()
    response = author_client.post(delete_url_reverse, data=bad_words_data)
    comments_count = Comment.objects.all().count()
    assert comments_count == 0


def test_author_can_edit_comment(author_client, comment, news, detail_url_reverse,
...
def test_user_cant_delete_comment_of_another_user(not_author_client, delete_url_reverse):
